...
Public Class frmMantFacturas
        ...
        Try
            ...
            If dr.HasRows Then
                While dr.Read()
                    ...
                    cmbComprobantes.Items.Add(dr("descripcion").ToString)
                End While
                dr.Close()
            Else
                dr.Close()
    ...
    Sub CalcularTotal()
        Try
            If dgvfactura.Rows.Count > 0 Then
                ...
                For X = 0 To dgvfactura.Rows.Count - 2
    ...
    Private Sub print_PrintPage(ByVal sender As Object, ByVal e As PrintPageEventArgs)
        ...
        Try
            ...
            If objReader.HasRows Then
                ...
                yPos = yPos + 18
                e.Graphics.DrawString("Numero de Factura : " & txtnumfactura.Text, prFont, Brushes.Black, xPos, yPos)
                ...
                StrSql = "SELECT * FROM DETALLES_FACTURA WHERE ID_FACTURA =" & txtnumfactura.Text & " ORDER BY ID_DETALLES_FACTURA ASC"
                objCmd = New OleDbCommand(StrSql, Cnn)
                objReader = objCmd.ExecuteReader
                If objReader.HasRows Then
                    ...
                    While objReader.Read
                        ...
                        If Len(objReader("descripcion")) > 0 And Len(objReader("descripcion")) <= 28 Then
                            xPos = 38
                            e.Graphics.DrawString(objReader("descripcion"), prFont, Brushes.Black, xPos, yPos)
                        ElseIf Len(objReader("descripcion")) > 28 Then
                            Dim Desc1 As String
                            Dim Desc2 As String
                            Desc1 = Mid(objReader("descripcion"), 1, 28)
                            Desc2 = Mid(objReader("descripcion"), 29, 28)
    ...
    Private Sub btagregar_Click(sender As Object, e As EventArgs) Handles btagregar.Click
        ...
        Try
            ...
            ElseIf cmbProducto.Text = "SELECCIONA UNA OPCION ..." Then
                MsgBox("Debe seleccionar una opción en Descripción", MsgBoxStyle.Information, "Mantenimiento de Facturas")
                cmbProducto.Focus()
                Return
            End If

            If Not IsNumeric(txtprecio.Text) Then
                MsgBox("El precio debe ser numérico ", MsgBoxStyle.Information)
                txtprecio.Focus()
                Return
            End If

            If Not IsNumeric(txtcantidad.Text) Then
                MsgBox("Debe Digitar una Cantidad numérica", MsgBoxStyle.Information)
                txtcantidad.Focus()
                Return
            Else
            ...
            If txtcantidad.BackColor = Color.White Then
                dgvfactura.Rows.Add(Trim(txtRef.Text), cmbProducto.Text, txtcantidad.Text, FormatNumber(txtprecio.Text, 2),
            ...
            Else
                ...
                dgvfactura.Item(8, posicion).Value = cmbIdCondicion
    ...
    Private Sub btinsertar_Click(sender As Object, e As EventArgs) Handles btinsertar.Click
        Try
            ...
            Dim myQuery As String = "UPDATE factura SET id_cliente =" & txtIdCliente.Text _
                                        ...
                                        & "', id_comprobante ='" & IdComprobante & "',valida_hasta ='" & ValidaHasta & "', pagada ='" & Pagada & "' WHERE id_factura = " & CLng(txtnumfactura.Text)
            ...
            If RA <= 0 Then
                trans.Rollback()
                MsgBox("Error actualizando el encabezado de la factura", MsgBoxStyle.Critical)
                txtcliente.Focus()
                Return
            End If

            StrSql = "SELECT REFERENCIA, CANTIDAD FROM DETALLES_FACTURA WHERE ID_FACTURA =" & CLng(txtnumfactura.Text)
            objCmd = New OleDbCommand(StrSql, Cnn, trans)
            objReader = objCmd.ExecuteReader
            If objReader.HasRows Then
                While objReader.Read
                    StrSql = "UPDATE INVENTARIO SET EXISTENCIA = EXISTENCIA + " & CDbl(objReader("cantidad")) _
                    & " WHERE REFERENCIA ='" & objReader("referencia").ToString & "'"
                    objCmd = New OleDbCommand(StrSql, Cnn, trans)
            ...
            Dim myQuery2 As String = "DELETE * FROM detalles_factura WHERE id_factura = " & CLng(txtnumfactura.Text)
            ...
            For X = 0 To dgvfactura.Rows.Count
                If Len(Trim(dgvfactura(0, X).Value)) > 0 Then
                    ...
                    Dim myQuery1 As String = "INSERT INTO detalles_factura(id_factura, referencia, descripcion, costo, cantidad, precio, importe, itbis, valor)" _
                    ...
                    & CDec(dgvfactura(5, X).Value) & "," & CDec(dgvfactura(6, X).Value) & ")"
    ...
    Private Sub txtcliente_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtcliente.KeyPress, cmbcondicion.KeyPress
    ...
    Private Sub bteliminar_Click(sender As Object, e As EventArgs) Handles bteliminar.Click
        Try
            ...
            cmbProducto.Items.Clear() : txtprecio.Clear() : txtcantidad.Clear() : txtCosto.Clear()
            txtcantidad.Focus()
    ...
    Private Sub btnAnular_Click(sender As Object, e As EventArgs) Handles btnAnular.Click
        Try
            ...
            If MessageBox.Show("Esta seguro que desea anular esta factura", " Importante ",
                ...
                Dim queryupdate As String = "UPDATE factura SET estado = 'Anulada', usuario_anulo = " & IdUsuario _
                & " WHERE id_factura = " & CLng(txtnumfactura.Text)
                Dim cmb As OleDbCommand = New OleDbCommand(queryupdate, Cnn, trans)
                ...
                If RA > 0 Then
                    ...
                    MsgBox("Ok. La factura fue anulada", vbInformation)
                    Return
                Else
                    trans.Rollback() : TA = False
                    MsgBox("Error. No fue posible anular la factura", vbInformation)
                    Return
                End If
            End If
    ...
    Private Sub btnBuscar_Click(sender As Object, e As EventArgs) Handles btnBuscar.Click
        Try
            ...
            If IdComprobante > 0 Then
                ...
                cmb = New OleDbCommand(query, Cnn)
                dr = cmb.ExecuteReader()
                If dr.HasRows Then
                    dr.Read()
            ...
            FormatGrid()
            Dim query1 As String = "SELECT referencia, cantidad, descripcion, costo, precio," _
            & "importe, itbis, valor, id_condicion FROM detalles_factura WHERE id_factura = " & NF _
            & " ORDER BY id_detalles_factura ASC"
            Dim cmb1 As OleDbCommand = New OleDbCommand(query1, Cnn)
            dr = cmb1.ExecuteReader
            If dr.HasRows Then
                While dr.Read()
                    dgvfactura.Rows.Add(dr("referencia"), dr("descripcion"), dr("cantidad"), FormatNumber(dr("precio"), 2),
                    FormatNumber(dr("importe"), 2), FormatNumber(dr("itbis"), 2), FormatNumber(dr("valor"), 2), FormatNumber(dr("costo"), 2), dr("id_condicion"))
                End While
                dr.Close()
            Else
                dr.Close()
    ...
    Private Sub btbuscar_Click(sender As Object, e As EventArgs) Handles btbuscar.Click
        ...
        FbuscarCliente.Tag = "MantFactura"
        FbuscarCliente.Show()
    ...
    Private Sub btnOtra_Click(sender As Object, e As EventArgs) Handles btnOtra.Click
        ...
        FbuscarFactura.MdiParent = Me.MdiParent
        FbuscarFactura.Show()
    End Sub

    Private Sub frmMantFacturas_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyValue = Keys.F2 Then
            FBuscarProductos.Tag = "MantFactura"
